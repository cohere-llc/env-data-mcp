"""Unit tests for ssurgo_subsurface_barriers_* tools (Type 3)."""
...
def test_subsurface_barriers_query_success_structure(httpx_mock):
    httpx_mock.add_response(method="POST", url=_SDA_URL, text=BARRIERS_XML)
...
def test_subsurface_barriers_query_non_us_returns_no_coverage(httpx_mock):
    ...
    result = ssurgo_subsurface_barriers_point_query(latitude=48.8566, longitude=2.3522)
    assert result["_meta"]["success"] is True
    assert result["data"] == []
